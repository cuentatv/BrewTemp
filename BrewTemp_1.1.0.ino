...
#define VERSION "1.1.0" // BrewTemp version
...
//  my_Main += "<body><div class=\"container\"><h2>BrewTemp Setup</h2><form method=\"get\" action=\"showForm\"><button class=\"btn\" type=\"submit\" formaction=\"factoryDefaults\">RESET TO FACTORY DEFAULTS</button><button class=\"btn\" type=\"submit\">CONFIGURATION</button></form></div></body></html>";
  my_Main += "<body><div class=\"container\"><h2>BrewTemp Setup</h2><form method=\"get\" action=\"showForm\"><fieldset><legend><b>RESET TO FACTORY DEFAULTS</b></legend><p>This implies to remove internal configuration file, format internal Flash Memory and create an empty configuration file.</p><p>This action will take about 4 minutes to be completed. Don't reset neither turn it off. </p><button class=\"btn\" type=\"submit\" formaction=\"factoryDefaults\">RESET TO FACTORY DEFAULTS</button></fieldset><fieldset><legend><b>BrewTemp Configuration</b></legend><p>BrewTemp gets several configuration parameters than can not be handled directed by Ubidots IoT platform, such as network setup (SSID and passphrase), Ubitdots Key-Token and device name. Other parameters as PID setting could be handled by Ubidots but are omitted due to overhead to manage too many control parameters. Only the most important temperature parameters can be modified at configuration mode as well as at runtime from Ubidots, these parameters are temperature, offset and operation mode.</p><p>This action will take about 30 seconds, after that BrewTemp device needs to be reseted.</p><button class=\"btn\" type=\"submit\">CONFIGURATION</button></fieldset></form></div></body></html>";
...
Serial.println("Settings");
  ...
  my_TempSet = server.arg("my_TempSet").c_str();
  my_TempOffset = server.arg("my_TempOffset").c_str();
  my_SensorOffset = server.arg("my_SensorOffset").c_str();
  my_IspindelOffset = server.arg("my_IspindelOffset").c_str();
  my_Mode = server.arg("my_Mode").c_str();
  my_KpHeater = server.arg("my_KpHeater").c_str();
  my_KpFreezer = server.arg("my_KpFreezer").c_str();
  my_KiHeater = server.arg("my_KiHeater").c_str();
  my_KiFreezer = server.arg("my_KiFreezer").c_str();
  saveConfig();
...
Serial.println("Factory Defaults");
  ...
  saveConfig();
//  Serial.println(" System is going to be restarted in 60 secs .....");
//  delay(60000);
//  ESP.restart();
}
// ********************************************************
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  pstr = strstr( topic, my_Name.c_str());
  if(pstr != NULL) {
...
void initWifiClient() {
  ...
  setPoint = (double)stringTofloat((char*)my_TempSet.c_str());
  offSet = stringTofloat((char*)my_TempOffset.c_str());
