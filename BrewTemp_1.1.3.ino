...
#define VERSION "1.1.3" // BrewTemp version


ESP8266WebServer server(HTTPPORT);
DoubleResetDetector drd(DRD_TIMEOUT, DRD_ADDRESS);

uint32_t my_doubleResetDetectorFlag;
uint32_t my_WifiDetectorFlag;
...
unsigned long currentMillis = millis();
  ...
  if (sendFlag) {
    ...
    ts.add(1, BLINK_INTERVAL*15, [&](void *) { blink1(); }, nullptr, true);
    sendFlag = false;
  }    
}
// ********************************************************
...
void setup(void) {
  ...
  if (drd.detectDoubleReset()) {
    ...
    initConf();
    ts.add(2, BLINK_INTERVAL*300, [&](void *) { wifiAPUp(); }, nullptr, true);
  }
  else {
